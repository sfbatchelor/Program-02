...
vec4 circularMasks(vec2 uv, vec4 colour)
{
	...
	vec2 id = floor(st);
	vec2 mid = id + floor(cos(uTime/10)*10);
	float n = n21(id+fract(sin(uTime*n21(mid)))*.0000004);
	float power = mix(.9, 1.5, n);
	...
	//random number based on id
	id = floor(st);
	mid = id + floor(sin(uTime/100)*0.00001);
	n = n21(id+(cos(uTime*n21(mid*id)))*.00000005);
	power = mix(.9, 1.0, n);
	rmag = mix(2.6,2.1,n);

    pos = fract(st)*1.3;
	pos -= vec2(0.2,.5);
	radius = length(pos)*rmag;
	angle = atan(pos.y, pos.x);
	val = cos(angle);
	cMask = ( 1.-smoothstep(val, val+0.8,radius) );
	val = (angle*10);
